...
export class VuexPersistence<S> implements PersistOptions<S> {
  ...
  public constructor(options?: PersistOptions<S>) {
    ...
    /** 1. Check to see if storage is localStorage or sessionStorage
     *  2. If it is, use options.compression
    ...
    this.compression = !w
      ? false
      : this.storage === w.localStorage || this.storage === w.sessionStorage
    ...
    if (this.asyncStorage) {
        ...
        options.restoreState != null
          ...
          : (key: string, storage: AsyncStorage) =>
              storage
                ...
                .then((value) =>
                  this.compression
                    ? decompressFromUTF16(value as string)
                    : value
                )
                .then((value) =>
          ...
          : (key: string, state: {}, storage: AsyncStorage) =>
              storage.setItem(
                ...
                this.asyncStorage
                  ...
                  : compressToUTF16(JSON.stringify(state) as any)
              );
      ...
      this.plugin = (store: Store<S>) => {
         ...
         * @since 2.1.0
         */
        (store as any).restored = (this.restoreState(
          this.key,
          this.storage
        ) as Promise<S>).then((savedState) => {
          /**
